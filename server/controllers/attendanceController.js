...
export const getBatchesByCollege = async (req, res) => {
  try {
    console.log("fetching college batches");
    const { collegeName } = req.query;
    if (!collegeName) return res.status(400).json({ error: "collegeName required" });
...
export const getStudentsByProgramTech = async (req, res) => {
  try {
    ...
    // include teacher field and populate basic teacher info so frontend can filter by teacher id
    const students = await User.find(query)
      .select("name email usn batch programName technology profilePic teacher")
...
const escapeRgx = (text) => text.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&");

export const getStudentsforTeacher = async (req, res) => {
  try {
    const { collegeName, batch, programName, technology, teacherId } = req.query;
    console.log("teacher id",teacherId)
    if (!programName || !technology)
      return res.status(400).json({ error: "programName and technology required" });
    ...
    if (collegeName) {
      query.collegeName = new RegExp(`^${escapeRgx(collegeName.trim())}$`, "i");
    }
    if (batch) {
      query.batch = batch;
    }

    // Fetch students belonging to the given teacher
    const students = await User.find(query)
  .select("name email usn batch programName technology profilePic teacher homeworkStatus")
  .populate("homeworkStatus.markedBy", "firstname lastname email phoneNo");

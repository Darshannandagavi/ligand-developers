...
const AdminDashboard = () => {
  ...
  const fetchStudents = async () => {
    try {
      ...
      const res = await axios.get("https://ligand-dev-4.onrender.com/api/users", {
    ...
    const fetchOptions = async () => {
      try {
        ...
        const responses = await Promise.all(
          types.map((type) =>
            axios.get(`https://ligand-dev-4.onrender.com/api/options/${type}`)
          )
        );
  ...
  const handleMakeBatchPassout = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        "https://ligand-dev-4.onrender.com/api/users/make-passout",
        payload,
        { headers: { Authorization: token ? `Bearer ${token}` : "" } }
      );
  ...
  const renderStudentCard = (student) => (
    <div key={student._id} className="student-card">
      <img
        src={
          student.profilePic
            ? `https://ligand-dev-4.onrender.com/uploads/${student.profilePic}`
            : "/default_user.jpeg"
        }
        alt="Profile"

...
export default function ManageExamAndNoteVisibility() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch exams
        const examsRes = await axios.get("http://localhost:8000/api/exams/examsforadmin");
        setExams(examsRes.data);

        // Fetch notes
        const notesRes = await axios.get("http://localhost:8000/api/notes/admin");
        setNotes(notesRes.data);

        // Fetch colleges added by admin
        const collegesRes = await axios.get("http://localhost:8000/api/options/collegeName");
  ...
  const toggleExamCollege = async (examId, collegeName) => {
    try {
      await axios.put(`http://localhost:8000/api/exams/exams/${examId}/college`, { collegeName });
      const updated = await axios.get("http://localhost:8000/api/exams/examsforadmin");
  ...
  const toggleNoteCollege = async (noteId, collegeName) => {
    try {
      await axios.put(`http://localhost:8000/api/notes/${noteId}/college`, { collegeName });
      const updated = await axios.get("http://localhost:8000/api/notes/admin");
